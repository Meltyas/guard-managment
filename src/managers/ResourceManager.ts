...
export class ResourceManager {
  ...
  public createResource(data: CreateResourceData): Resource {
    ...
    console.log(`ResourceManager | Created resource: ${resource.name} (${resource.id})`);
    return resource;
  }
  ...
  public updateResource(id: string, updates: UpdateResourceData): Resource {
    ...
    console.log(`ResourceManager | Updated resource: ${updated.name} (${id})`);
    return updated;
  }
  ...
  public deleteResource(id: string): boolean {
    const resource = this.resources.get(id);
    if (!resource) {
      console.warn(`ResourceManager | Resource not found: ${id}`);
      return false;
    }
    ...
    console.log(`ResourceManager | Deleted resource: ${resource.name} (${id})`);
    return true;
  }
  ...
  private async loadResourceData(): Promise<void> {
    try {
      ...
      console.log(`ResourceManager | Loaded ${data.length} resources`);
    } catch (error) {
      console.warn('ResourceManager | Error loading resource data:', error);
    }
  }
  ...
  private async saveResourceData(): Promise<void> {
    try {
      ...
      console.log(`ResourceManager | Saved ${data.length} resources`);
    } catch (error) {
